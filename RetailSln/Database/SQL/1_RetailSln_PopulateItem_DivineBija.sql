USE [RetailSln]
GO
--1_RetailSln_PopulateItem_DivineBija.sql
--Dec 20 2024, Apr 2 2024, Apr 21 2024
--Created as new file - This is common for all instances
...
--
        ...
        WHERE AspNetRoleName IN('DEFAULTROLE', 'APPLADMN1', 'SYSTADMIN', 'MARKETINGROLE', 'PRIESTROLE')
        AND ParentCategoryId NOT IN(102, 120)--'Bulk Orders', 'Wholesale Orders'
        UNION
        SELECT DISTINCT @ClientId AS ClientId, AspNetRoleName, ItemId, ItemMasterId, ItemMasterSeqNum, ItemSeqNum, ParentCategoryId
        FROM #TEMP1A, ArchLib.AspNetRole
        WHERE AspNetRoleName IN('BULKORDERSROLE', 'APPLADMN1', 'SYSTADMIN', 'MARKETINGROLE', 'WHOLESALEROLE', 'PRIESTROLE')
...
--CategoryItemMasterHier
        ...
        SELECT @RowCount = COUNT(*) FROM DivineBija_CategoryHiers
        IF @RowCount = 1
            ...
            WHERE #TEMP2A.ParentCategoryId <> 100
...
--CategoryItemMasterHier & CategoryItemHier
--SELECT * FROM #TEMP1A
--SELECT * FROM #TEMP2A WHERE AspNetRoleName = 'APPLADMN1' AND ParentCategoryId = 104 AND ItemMasterId = 24
--End CategoryHier(s)
...
--PRINT CAST(@ItemSpecMasterId AS VARCHAR) + ' 1 ' + @SqlStmt
                EXEC(@SqlStmt)
...
--PRINT CAST(@ItemSpecMasterId AS VARCHAR) + ' 4 ' + @SqlStmt
                EXEC(@SqlStmt)
                IF @CodeTypeId IS NOT NULL
                BEGIN
                    SET @SqlStmt = ''
...
--PRINT CAST(@ItemSpecMasterId AS VARCHAR) + ' 5 ' + @SqlStmt
                    EXEC(@SqlStmt)
                END
                IF ISNULL(@ShowValueColumnName, '') <> ''
                BEGIN
...
--PRINT CAST(@ItemSpecMasterId AS VARCHAR) + ' 6 ' + @SqlStmt
                    EXEC(@SqlStmt)
                END
            END
            FETCH ItemSpecMasterCursor INTO @ItemSpecMasterId, @BookFlag, @ProductFlag, @ValueColumnName, @UnitColumnName
