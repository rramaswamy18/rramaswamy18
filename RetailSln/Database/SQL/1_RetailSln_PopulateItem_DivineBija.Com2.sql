...
--
        INSERT RetailSlnSch.Item
              (
               ClientId, ItemMasterId, ItemRate, ItemRateMSRP, ItemSeqNum, ItemStarCount, ItemStatusId, ItemStockStatusId, ItemUniqueDesc,
               ProductItemId, UploadImageFileName
              )
--Item & Item Bundle
        SELECT @ClientId AS ClientId, ItemMaster.ItemMasterId, -1 AS ItemRate, -1 AS ItemRateMSRP
              ,CASE ISNUMERIC([Spec Seq]) WHEN 1 THEN CAST([Spec Seq] AS INT) ELSE 0 END AS ItemSeqNum, 5 AS ItemStarCount
              ,100 AS ItemStatusId, 100 AS ItemStockStatusId, UniqueDescription AS ItemUniqueDesc, ItemId AS ProductItemId
              ,ImageFileName AS UploadImageFileName
          FROM dbo.DivineBija_Products
    INNER JOIN RetailSlnSch.ItemMaster
         ...
         WHERE [USA Active] = 1
      ORDER BY ItemUniqueDesc, ItemSeqNum
--
        INSERT RetailSlnSch.Item
              (
               ClientId, ItemMasterId, ItemRate, ItemRateMSRP, ItemSeqNum, ItemStarCount, ItemStatusId, ItemStockStatusId, ItemUniqueDesc,
...
--Books
        SELECT @ClientId AS ClientId, ItemMaster.ItemMasterId, ItemMaster.ItemMasterId, -1 AS ItemRate, -1 AS ItemRateMSRP
              ,CASE ISNUMERIC([Spec Seq]) WHEN 1 THEN CAST([Spec Seq] AS INT) ELSE 0 END AS ItemSeqNum, 5 AS ItemStarCount
              ,100 AS ItemStatusId, UniqueDescription AS ItemUniqueDesc, ItemId AS ProductItemId, Image1 AS UploadImageFileName
          FROM dbo.DivineBija_Books
    INNER JOIN RetailSlnSch.ItemMaster
...
--Begin ItemInfo
        ...
        ORDER BY ItemId, SeqNum
--End ItemInfo
--Begin Category, CategoryHier, CategoryRole, CategoryItemMasterHier
--Category
        TRUNCATE TABLE RetailSlnSch.Category
        INSERT RetailSlnSch.Category
              (
               CategoryId, ClientId, AssignSubCategory, AssignItem, CategoryDesc, CategoryLongDesc, CategoryNameDesc, CategoryStatusId
              ,CategoryTypeId, DefaultCategory, ImageExtension, MaxPerPage, ViewName
...
--CategoryCategoryHier
        ...
        SELECT @ClientId, Category.CategoryId, ParentCategory.CategoryId AS ParentCategoryId, [Seq Num]
          FROM DivineBija_CategoryHiers
    INNER JOIN RetailSlnSch.Category ON [Category Name Desc] = CategoryNameDesc
    INNER JOIN RetailSlnSch.Category AS ParentCategory ON [Parent CategoryName Desc] = ParentCategory.CategoryNameDesc
      ORDER BY ParentCategory.CategoryId, DivineBija_CategoryHiers.[Seq Num]
        TRUNCATE TABLE RetailSlnSch.AspNetRoleCategory
        INSERT RetailSlnSch.AspNetRoleCategory(ClientId, AspNetRoleName, CategoryId, SeqNum)
        SELECT @ClientId, [Role Name] AS AspNetRoleName, CategoryId, [Seq Num]
          FROM DivineBija_CategoryRoles
    INNER JOIN RetailSlnSch.Category ON [Category Name Desc] = CategoryNameDesc
      ORDER BY AspNetRoleName, [Seq Num]
--CategoryItemMasterHier
        TRUNCATE TABLE RetailSlnSch.CategoryItemMasterHier
        INSERT RetailSlnSch.CategoryItemMasterHier(ClientId, ItemMasterId, ParentCategoryId, SeqNum)
        SELECT DISTINCT @ClientId AS ClientId, ItemMaster.ItemMasterId, Category.CategoryId AS ParentCategoryId, DivineBija_Products.SeqNumManual AS SeqNum
...
--CategoryItemMasterHier
--End Category, CategoryHier, CategoryRole, CategoryItemMasterHier
