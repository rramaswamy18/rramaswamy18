...
namespace RetailSlnBusinessLayer
{
    public partial class RetailSlnBL
    {
        ...
        #region New code Restored from GitHub
        // GET: ItemCatalog
        ...
        // POST : ShoppingCartComments
        public void ShoppingCartComments(ShoppingCartModel shoppingCartModel, string indexParm, string bundleIndexParm, string orderComments, SessionObjectModel sessionObjectModel, SessionObjectModel createForSessionObject, Controller controller, HttpSessionStateBase httpSessionStateBase, ModelStateDictionary modelStateDictionary, long clientId, string ipAddress, string execUniqueId, string loggedInUserId)
        {
            string methodName = MethodBase.GetCurrentMethod().Name;
            ExceptionLogger exceptionLogger = Utilities.CreateExceptionLogger(Utilities.GetApplicationValue("ApplicationName"), ipAddress, execUniqueId, loggedInUserId, Assembly.GetCallingAssembly().FullName, Assembly.GetExecutingAssembly().FullName, MethodBase.GetCurrentMethod().DeclaringType.ToString());
            exceptionLogger.LogInfo(methodName, Utilities.GetCallerLineNumber(), "00000000 :: Enter");
            try
            {
                ...
                if (bundleIndex == -1)
                {
                    shoppingCartModel.ShoppingCartItemModels[index].OrderComments = orderComments;
                }
                else
                {
                    shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].OrderComments = orderComments;
                }
                exceptionLogger.LogInfo(methodName, Utilities.GetCallerLineNumber(), "00090000 :: Exit");
                return;
            }
        ...
        // GET : ShoppingCartQty
        public void ShoppingCartQty(ShoppingCartModel shoppingCartModel, string indexParm, string bundleIndexParm, string orderQtyParm, SessionObjectModel sessionObjectModel, SessionObjectModel createForSessionObject, Controller controller, HttpSessionStateBase httpSessionStateBase, ModelStateDictionary modelStateDictionary, long clientId, string ipAddress, string execUniqueId, string loggedInUserId)
        {
            string methodName = MethodBase.GetCurrentMethod().Name;
            ExceptionLogger exceptionLogger = Utilities.CreateExceptionLogger(Utilities.GetApplicationValue("ApplicationName"), ipAddress, execUniqueId, loggedInUserId, Assembly.GetCallingAssembly().FullName, Assembly.GetExecutingAssembly().FullName, MethodBase.GetCurrentMethod().DeclaringType.ToString());
            exceptionLogger.LogInfo(methodName, Utilities.GetCallerLineNumber(), "00000000 :: Enter");
            try
            {
                int.TryParse(indexParm, out int index);
                int.TryParse(bundleIndexParm, out int bundleIndex);
                long.TryParse(orderQtyParm, out long orderQty);
                ShoppingCartItemModel shoppingCartItemModel = shoppingCartModel.ShoppingCartItemModels[index];
                ...
                else
                {
                    ...
                    shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].OrderQty = orderQty;
                    shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].OrderQtyParm = orderQty.ToString();

                    productOrVolumetricWeightUnit = shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].ProductOrVolumetricWeightUnit.Value;
                    weightCalcValueUnit = shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].WeightCalcValueUnit.Value;
                    weightValueUnit = shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].WeightValueUnit.Value;

                    productOrVolumetricWeight = orderQty * shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].ProductOrVolumetricWeightUnit.Value;
                    weightCalcValue = orderQty * shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].WeightCalcValueUnit.Value;
                    weightValue = orderQty * shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].WeightValueUnit.Value;

                    shoppingCartItemModel.OrderAmount -= shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].OrderAmount;
                    shoppingCartItemModel.ProductOrVolumetricWeight -= shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].ProductOrVolumetricWeight;
                    shoppingCartItemModel.WeightCalcValue -= shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].WeightCalcValue;
                    shoppingCartItemModel.WeightValue -= shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].WeightValue;

                    shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].OrderAmount = itemRate * orderQty;
                    shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].OrderAmountBeforeDiscount = itemRateBeforeDiscount * orderQty;
                    shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].OrderAmountFormatted = shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].OrderAmount.Value.ToString(RetailSlnCache.CurrencyDecimalPlaces, RetailSlnCache.CurrencyCultureInfo).Replace(" ", "");
                    shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].OrderAmountRounded = (long)(shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].OrderAmount * 100);
                    shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].ProductOrVolumetricWeight = productOrVolumetricWeight;
                    shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].WeightCalcValue = 0;//weightCalcValue;
                    shoppingCartModel.ShoppingCartItemModels[index].ShoppingCartItemBundleModels[bundleIndex].WeightValue = 0;//weightValue;
        ...
        //    try
        //    {
        //        #region Delete this later while uncommenting the below
        //        //ItemCatalogItemCreate("DEFAULTROLE", 0, itemCatalogFilesPath, sessionObjectModel, createForSessionObjectModel, controller, httpSessionStateBase, modelStateDictionary, clientId, ipAddress, execUniqueId, loggedInUserId);
        //        #endregion
        //        #region Comment it for now - uncomment later
        //        long corpAcctId;
        //        string aspNetRoleName;
        //        List<CodeDataModel> corpAcctTypes = LookupCache.CodeDataModels.FindAll(x => x.CodeTypeId == 204);
        //        CodeDataModel corpAcctType;
        //        Dictionary<string, AspNetRoleKVPModel> aspNetRoleKVPs;
        //        aspNetRoleName = "APPLADMN1";
        //        //ItemCatalogItemCreate_A(aspNetRoleName, itemCatalogFilesPath, sessionObjectModel, createForSessionObjectModel, controller, httpSessionStateBase, modelStateDictionary, clientId, ipAddress, execUniqueId, loggedInUserId);
        //        foreach (var corpAcctModel in RetailSlnCache.CorpAcctModels)
        //        {
        //            corpAcctId = corpAcctModel.CorpAcctId.Value;
        //            if (corpAcctId == 0)
        ...
        //}
        //// PRIVATE: ItemCatalogItemCreate
        //private void ItemCatalogItemCreate_A(string aspNetRoleName, string itemCatalogFilesPath, SessionObjectModel sessionObjectModel, SessionObjectModel createForSessionObjectModel, Controller controller, HttpSessionStateBase httpSessionStateBase, ModelStateDictionary modelStateDictionary, long clientId, string ipAddress, string execUniqueId, string loggedInUserId)
        //{
        //    long corpAcctId, parentCategoryId = 100;
        //    List<CategoryItemMasterHierModel> categoryCategoryItemMasterHierModels = RetailSlnCache.AspNetRoleParentCategoryCategoryModels[aspNetRoleName][0];
        //    var aspNetRoleParentCategoryItemMasterModels = RetailSlnCache.AspNetRoleParentCategoryItemMasterModels[aspNetRoleName];
        //    var categoryItemMasterHierModels = aspNetRoleParentCategoryItemMasterModels[parentCategoryId];
        //    ItemCatalogFileModel itemCatalogFileModel;
        //    long itemCount;
        //    string htmlFileName, htmlString;
        //    StreamWriter streamWriter;
        //    ArchLibBL archLibBL = new ArchLibBL();
        //    foreach (var corpAcctModel in RetailSlnCache.CorpAcctModels)
        //    {
        ...
        //            ItemMasterCount = categoryItemMasterHierModels.Count,
        //            ItemCount = itemCount,
        //            ParentCategoryDesc = "All Item(s)",
        //            ParentCategoryId = null,
        //            PdfFlag = false,
        //        };
        //        htmlFileName = itemCatalogFilesPath + $@"\ItemCatalog_A_{aspNetRoleName}_{corpAcctId}_{parentCategoryId}.html";
        //        htmlString = archLibBL.ViewToHtmlString(controller, "_ItemCatalogFileSearch", itemCatalogFileModel);
        //        streamWriter = new StreamWriter(htmlFileName);
        //        streamWriter.Write(htmlString);
        //        streamWriter.Close();
        //    }
        ...
        //            streamWriter.Write(htmlString);
        //            streamWriter.Close();
        //            //if (aspNetRoleName == "APPLADMN1" && parentCategoryId == 100)
        //            //{
        //            //    htmlFileName = itemCatalogFilesPath + $@"\ItemCatalog_{aspNetRoleName}_{corpAcctId}_{parentCategoryId}_A.html";
        //            //    htmlString = archLibBL.ViewToHtmlString(controller, "_ItemCatalogFileSearch", itemCatalogFileModel);
        //            //    streamWriter = new StreamWriter(htmlFileName);
        //            //    streamWriter.Write(htmlString);
        //            //    streamWriter.Close();
        ...
        //            if (aspNetRoleName == "DEFAULTROLE" && corpAcctId == 0 && parentCategoryId == 100)
        //            {
        //                //exceptionLogger.LogInfo(methodName, Utilities.GetCallerLineNumber(), "00002000 :: Html Begin", "AspNetRoleName", aspNetRoleName, "CorpAcctId", corpAcctId.ToString(), "ParentCategoryId", parentCategoryId.ToString());
        //                exceptionLogger.LogInfo(methodName, Utilities.GetCallerLineNumber(), "00002000 :: Pdf Begin", "AspNetRoleName", aspNetRoleName, "CorpAcctId", corpAcctId.ToString(), "ParentCategoryId", parentCategoryId.ToString());
        //                itemCatalogFileModel = new ItemCatalogFileModel
        //                {
        //                    CategoryCategoryItemMasterHierModels = categoryCategoryItemMasterHierModels,
        ...
        //                };
        //                //htmlFileName = itemCatalogFilesPath + $@"\ItemCatalog_{aspNetRoleName}_{corpAcctId}_{parentCategoryId}_A.html";
        //                //htmlString = archLibBL.ViewToHtmlString(controller, "_ItemCatalogFileData", itemCatalogFileModel);
        //                //streamWriter = new StreamWriter(htmlFileName);
        //                //streamWriter.Write(htmlString);
        //                //streamWriter.Close();
        //                //exceptionLogger.LogInfo(methodName, Utilities.GetCallerLineNumber(), "00003000 :: Html End");
        //                //itemCatalogFileModel.ItemDiscountModels = null;
        //                //itemCatalogFileModel.PdfFlag = true;
        //                htmlString = archLibBL.ViewToHtmlString(controller, "ItemCatalogPdf", itemCatalogFileModel);
        //                var pDFFullFileName = itemCatalogFilesPath + $@"\ItemCatalog.pdf";
        //                pDFUtility.GeneratePDFFromHtmlString(htmlString, pDFFullFileName);
        //                exceptionLogger.LogInfo(methodName, Utilities.GetCallerLineNumber(), "00003000 :: Pdf End");
        //            }
        //            //exceptionLogger.LogInfo(methodName, Utilities.GetCallerLineNumber(), "00005000 :: End");
        //        }
        //        return;
        //    }
        //    catch (Exception exception)
        ...
        private void CreateShoppingCartItemModel(ref ShoppingCartItemModel shoppingCartItemModel, AddToCartModel addToCartModel, SessionObjectModel sessionObjectModel, SessionObjectModel createForSessionObject, Controller controller, HttpSessionStateBase httpSessionStateBase, ModelStateDictionary modelStateDictionary, long clientId, string ipAddress, string execUniqueId, string loggedInUserId)
        {
            ...
            float itemRate, itemRateBeforeDiscount, productOrVolumetricWeight, weightCalcValue, weightValue, productOrVolumetricWeightUnit, weightCalcValueUnit, weightValueUnit;
            ...
            if (shoppingCartItemModel == null)
            {
                ...
                {
                    shoppingCartItemModel.ProductOrVolumetricWeightUnit = float.Parse(itemModel.ItemItemSpecModels["ProductOrVolumetricWeight"].ItemSpecValue);
                    shoppingCartItemModel.WeightCalcValueUnit = float.Parse(itemModel.ItemItemSpecModels["ProductOrVolumetricWeight"].ItemSpecValue);
                    shoppingCartItemModel.WeightValueUnit = float.Parse(itemModel.ItemItemSpecModels["ProductWeight"].ItemSpecValue);

                    itemRateBeforeDiscount = addToCartModel.ItemModel.ItemRate.Value;

                    productOrVolumetricWeightUnit = shoppingCartItemModel.ProductOrVolumetricWeightUnit.Value;
                    weightCalcValueUnit = 0;//shoppingCartItemModel.WeightCalcValueUnit.Value;
                    weightValueUnit = 0;//addToCartModel.OrderQty * shoppingCartItemModel.WeightValueUnit.Value;

                    productOrVolumetricWeight = addToCartModel.OrderQty * shoppingCartItemModel.ProductOrVolumetricWeightUnit.Value;//float.Parse(itemModel.ItemItemSpecModels["ProductOrVolumetricWeight"].ItemSpecValue);
                    ...
                    if (addToCartModel.ItemModel.ItemTypeId == ItemTypeEnum.ItemBundle)
                    {
                        ...
                        for (i = 0; i < shoppingCartItemBundleModelsFromCache.Count; i++)
                        {
                            ...
                            shoppingCartItemModel.ShoppingCartItemBundleModels.Add(shoppingCartItemBundleModel = CreateShoppingCartItemBundleModelObject(i, addToCartModel, shoppingCartItemBundleModelsFromCache, shoppingCartItemBundleModelsFromCache[i].ItemModel, sessionObjectModel, createForSessionObject, controller, httpSessionStateBase, modelStateDictionary, clientId, ipAddress, execUniqueId, loggedInUserId));
                            shoppingCartItemBundleModel.ProductOrVolumetricWeightUnit = float.Parse(shoppingCartItemBundleModelsFromCache[i].ItemModel.ItemItemSpecModels["ProductOrVolumetricWeight"].ItemSpecValue);
                            shoppingCartItemBundleModel.WeightCalcValueUnit = float.Parse(shoppingCartItemBundleModelsFromCache[i].ItemModel.ItemItemSpecModels["ProductOrVolumetricWeight"].ItemSpecValue);
                            shoppingCartItemBundleModel.WeightValueUnit = float.Parse(shoppingCartItemBundleModelsFromCache[i].ItemModel.ItemItemSpecModels["ProductWeight"].ItemSpecValue);

                            shoppingCartItemBundleModel.ProductOrVolumetricWeight = shoppingCartItemBundleModel.ProductOrVolumetricWeightUnit;
                            shoppingCartItemBundleModel.WeightCalcValue = shoppingCartItemBundleModel.WeightCalcValueUnit;
                            shoppingCartItemBundleModel.WeightValue = shoppingCartItemBundleModel.WeightValueUnit;
                        }
                    }
                }
                else
                {
                    ...
                    productOrVolumetricWeightUnit = 0;
                    weightCalcValueUnit = 0;
                    weightValueUnit = 0;
                    ...
                    for (i = 0; i < shoppingCartItemBundleModelsFromCache.Count; i++)
                    {
                        ...
                        productOrVolumetricWeightUnit += shoppingCartItemBundleModelsFromCache[i].ProductOrVolumetricWeight.Value;
                        weightCalcValueUnit += 0;//shoppingCartItemBundleModelsFromCache[i].WeightCalcValueUnit.Value;
                        weightValueUnit += 0;//shoppingCartItemBundleModelsFromCache[i].WeightValueUnit.Value;

                        productOrVolumetricWeight += addToCartModel.ShoppingCartItemBundleModels[i].OrderQty.Value * shoppingCartItemBundleModelsFromCache[i].ProductOrVolumetricWeight.Value;
            ...
            else
            {
                ...
                else
                {
                    ...
                    for (i = 0; i < shoppingCartItemBundleModelsFromCache.Count; i++)
                    {
                        ...
                        productOrVolumetricWeight += shoppingCartItemModel.ShoppingCartItemBundleModels[i].OrderQty.Value * shoppingCartItemBundleModelsFromCache[i].ProductOrVolumetricWeight.Value;
        ...
        // PRIVATE: OrderWIPAdd
        private void ShoppingCartWIPAdd(bool doNotBreakBundle, long corpAcctLocationId, ref float itemSeqNum, ShoppingCartModel shoppingCartModel, ShoppingCartItemModel shoppingCartItemModel, SqlConnection sqlConnection, SessionObjectModel sessionObjectModel, SessionObjectModel createForSessionObject, Controller controller, HttpSessionStateBase httpSessionStateBase, ModelStateDictionary modelStateDictionary, long clientId, string ipAddress, string execUniqueId, string loggedInUserId)
        {
            string methodName = MethodBase.GetCurrentMethod().Name;
            ExceptionLogger exceptionLogger = Utilities.CreateExceptionLogger(Utilities.GetApplicationValue("ApplicationName"), ipAddress, execUniqueId, loggedInUserId, Assembly.GetCallingAssembly().FullName, Assembly.GetExecutingAssembly().FullName, MethodBase.GetCurrentMethod().DeclaringType.ToString());
            exceptionLogger.LogInfo(methodName, Utilities.GetCallerLineNumber(), "00000000 :: Enter");
            ...
            {
                //int x = 1, y = 0, z = x / y;
                if (shoppingCartModel.ShoppingCartWIPHdrModel == null)
                {
                    ShoppingCartWIPHdrModel shoppingCartWIPHdrModel = new ShoppingCartWIPHdrModel
                    {
                        ClientId = clientId,
                    ...
                    ApplicationDataContext.ShoppingCartWIPHdrAdd(shoppingCartWIPHdrModel, ApplicationDataContext.SqlConnectionObject, clientId, ipAddress, execUniqueId, loggedInUserId);
                    shoppingCartModel.ShoppingCartWIPHdrModel = shoppingCartWIPHdrModel;
                }
                ...
                shoppingCartWIPModel = new ShoppingCartWIPModel
                {
                    ...
                    OrderQty = shoppingCartItemModel.OrderQty.Value,
                    ParentItemId = shoppingCartItemModel.ParentItemId.Value,
                    ShoppingCartWIPHdrId = shoppingCartModel.ShoppingCartWIPHdrModel.ShoppingCartWIPHdrId,
                };
                ...
                if (shoppingCartItemModel.ShoppingCartItemBundleModels != null && shoppingCartItemModel.ShoppingCartItemBundleModels.Count > 0)
                {
                    foreach (var shoppingCartItemBundleModel in shoppingCartItemModel.ShoppingCartItemBundleModels)
                    {
                        shoppingCartWIPModel = new ShoppingCartWIPModel
                        {
                            ...
                            DoNotBreakBundle = doNotBreakBundle,
                            ItemId = shoppingCartItemBundleModel.ItemId.Value,
                            OrderComments = shoppingCartItemBundleModel.OrderComments,
                            OrderQty = shoppingCartItemBundleModel.OrderQty.Value,
                ...
                shoppingCartModel.ShoppingCartWIPHdrModel.SeqNum = itemSeqNum;
                exceptionLogger.LogInfo(methodName, Utilities.GetCallerLineNumber(), "00090000 :: Exit");
            }
            catch (Exception exception)
            {
        ...
        private void ShoppingCartWIPDel(long shoppingCartWIPHdrId, ShoppingCartItemModel shoppingCartItemModel, SessionObjectModel sessionObjectModel, SessionObjectModel createForSessionObject, Controller controller, HttpSessionStateBase httpSessionStateBase, ModelStateDictionary modelStateDictionary, long clientId, string ipAddress, string execUniqueId, string loggedInUserId)
        {
            ...
            try
            {
                ...
                {
                    ApplicationDataContext.ShoppingCartWIPDel(shoppingCartItemModel.ShoppingCartWIPId, ApplicationDataContext.SqlConnectionObject, clientId, ipAddress, execUniqueId, loggedInUserId);
                }
            }
            catch (Exception exception)
            {
